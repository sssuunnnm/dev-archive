---
title: 문자열 숫자 변환
command: to_string / stoi / stol / stoll / stod
...
- `stoi` 등은 변환 불가능한 문자열이 들어오면 예외(`invalid_argument`)를 던진다. 값이 대상 타입의 범위를 벗어나면 `out_of_range` 예외가 던져진다. 입력값이 항상 유효한 숫자·범위라고 확신할 수 없으면 `try-catch`로 감싸되, 두 예외를 모두 처리해야 한다.
