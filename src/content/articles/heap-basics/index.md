...
카드 `[1, 2, 3, 4]`로 위 패턴을 재생한다. 매번 힙에서 가장 작은 두 값을 꺼내 합치고, 그 합을 다시 힙에 넣는 과정을 그대로 따라간다. (구현 노트: 데모 내부에서는 `priority_queue`의 `pop`/`push` 대신 배열을 매번 정렬해 "가장 작은 두 값" 순서만 재현했다 — 값이 꺼내지고 들어가는 순서는 동일하지만 실제 힙 자료구조의 내부 동작을 그대로 보여주는 건 아니다.)
