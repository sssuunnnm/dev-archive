...
[Docker 기초 글](/infra/docker-basics/)에서 "OS 커널을 호스트와 공유하면서 프로세스 단위로 격리한다"고만 짚고 넘어갔는데, 이게 정확히 무슨 뜻인지, 가상 머신(VM)과는 뭐가 다른지는 따로 정리할 만하다. 인프라나 미들웨어 쪽 면접에서도 가상화 → 반가상화 → 컨테이너 순서로 격리 기술의 발전 과정을 묻는 경우가 흔하다.
