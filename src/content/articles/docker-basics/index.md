...
로컬, 팀원 PC, 서버가 OS나 라이브러리 버전이 조금씩 달라서 "분명 로컬에서는 됐는데 서버에서 안 된다"는 문제가 반복된다. 가상 머신(VM)으로 환경을 통째로 맞추면 해결되긴 하지만 VM 하나마다 OS 전체를 띄워야 해서 무겁고 느리다. Docker는 OS 커널을 호스트와 공유하면서 프로세스 단위로 격리해서, VM보다 훨씬 가볍게 같은 문제를 해결한다.
...
`Dockerfile`의 각 줄은 레이어(layer) 하나가 되고, 위에서 아래로 캐시된다. 그래서 자주 바뀌는 파일(`COPY . .`)은 뒤쪽에, 잘 안 바뀌는 의존성 설치(`package*.json` 복사 → `npm install`)는 앞쪽에 두면 코드만 바뀌었을 때 의존성 설치 레이어를 다시 안 돌려도 돼서 빌드가 빨라진다.
...
`docker run`은 이미지가 로컬에 없으면 자동으로 받아온 뒤 컨테이너를 만들고, `docker start`/`docker stop`은 이미 만들어진 컨테이너를 재사용한다는 차이가 있다.
...
- `docker system prune`처럼 한 번에 정리하는 명령어는 사용하지 않는 이미지/컨테이너/볼륨을 지우는데, 무엇을 지우는지 옵션(`-a`, `--volumes` 등)에 따라 범위가 크게 달라지니 옵션을 확인하지 않고 습관적으로 돌리지 않는다.
