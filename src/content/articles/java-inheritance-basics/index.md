...
`extends`로 부모의 필드·메서드를 물려받고, 부모 생성자는 `super()`로 호출한다 — 부모에 기본 생성자(매개변수 없는 생성자)가 있으면 생략해도 되지만, 없으면 반드시 명시해야 한다.

## 왜 (배경/문제 상황)

정보처리기사 실기를 준비하다가 상속 관련 코드 실행 결과를 묻는 문제에서 자주 막혔다. 특히 `super()`를 언제 생략할 수 있는지, 생성자가 여러 단계로 연결되면 실행 순서가 어떻게 되는지가 헷갈려서 정리한다.
...
여기서 헷갈리는 부분은 "`super()`를 꼭 써야 하는가"인데, 부모 클래스에 **기본 생성자(매개변수 없는 생성자)가 있는지**에 따라 갈린다.

| 부모 클래스 상태 | super 필요 여부 | 이유 |
|---|---|---|
| 생성자를 아예 안 만듦 (컴파일러가 기본 생성자 자동 생성) | 생략 가능 | 컴파일러가 `super()`를 자동으로 호출 |
| 기본 생성자를 직접 만듦 | 생략 가능 | 위와 동일 |
| 매개변수 있는 생성자만 만듦 | 반드시 명시 | 기본 생성자가 없어서 자동 호출이 불가능 |
...
`this()`로 다른 생성자를 계속 부르기만 할 수는 없다 (그러면 무한 루프가 된다). 그래서 체인의 끝에는 항상 `super()`가 있는 생성자가 있고, 결과적으로 **부모 생성자가 항상 가장 먼저 실행**된다.
...
- 부모 클래스에 매개변수 생성자만 있고 기본 생성자가 없는 상태에서 `super()`를 생략하면, "부모의 기본 생성자가 없다"는 컴파일 에러가 난다 — 시험에서 자주 나오는 함정이다.
