...
**해결**: SQL에서 집계 함수는 한 단계만 적용된다. "그룹별 평균"이 필요하면 그룹 안의 개별 값을 그대로 `AVG()`에 넣으면 되고, `GROUP BY`가 이미 그룹별로 나눠서 계산해주기 때문에 `SUM`으로 따로 합계를 낼 필요가 없다.
...
- 집계 함수는 중첩해서 쓸 수 없다. 그룹별 평균은 `AVG()` 하나로 충분하며, `GROUP BY`가 그룹 분리를 담당한다.
