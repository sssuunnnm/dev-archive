...
Redis가 "캐시 DB"로 가장 많이 알려져 있지만, 사실 캐시는 여러 역할 중 하나일 뿐이다. 세션 스토어, Pub/Sub 기반 메시징, 레이트 리미터, 랭킹(Sorted Set), 드물게는 RDB/AOF로 영속화해서 주 데이터베이스로 쓰이기도 한다 — 메모리 기반이라 빠르다는 특성 하나로 여러 문제에 재사용되는 것이다. 이 글은 그중 캐시 용도에 집중하고, 다른 역할들은 [별도 글](/infra/redis-beyond-caching/)에서 다룬다.
