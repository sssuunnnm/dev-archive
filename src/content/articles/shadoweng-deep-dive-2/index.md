...
기획 초기에는 게임 모드가 지금보다 훨씬 다양한 분기와 상호작용을 포함하는 형태였다. MVI를 검토하고 도입한 시점은 그 기획을 기준으로 한 판단이었다. 이후 일정 문제로 게임 모드 자체가 지금 형태로 단순화됐는데, 그 이야기는 뒤에서 따로 다룬다. 일단 최종적으로 남은 흐름부터 보면, 한 라운드 안에서 여러 단계를 거친다.
...
### 5. 기획이 단순해진 뒤에도 구조를 유지한 이유

여기서 솔직해질 필요가 있다. MVI를 도입하기로 판단한 시점의 게임 모드 기획과, 실제로 출시된 게임 모드(카운트다운→녹음→평가→결과)는 복잡도 차이가 크다. 개발 중반에 일정이 빠듯해지면서 게임 모드 자체를 지금 형태로 축소했는데, 그 시점엔 이미 State/Intent/Effect 구조와 Contract 파일이 만들어져 있었다.

이 시점에 선택지는 두 가지였다.

- 단순해진 최종 스펙에 맞춰 MVI를 걷어내고 State + Event로 되돌린다.
- 이미 동작하는 구조를 그대로 두고, 남은 일정을 신규 기능 완성도에 쓴다.

이미 짜여진 Contract·ViewModel을 걷어내는 리팩터링은 그 자체로 테스트·검증 비용이 들고, 남은 일정에 비해 얻는 이득(보일러플레이트 축소)이 크지 않다고 판단해 구조를 그대로 뒀다. 결과적으로 지금 시점에서 최종 스펙만 놓고 새로 설계한다면, `sealed class GamePlayPhase` 하나로 State를 정리하는 정도로도 충분했을 가능성이 크다. 이건 "잘못된 선택이었다"기보다, **기획이 확정되기 전에 구조를 먼저 정해야 하는 상황에서는 이후 스펙이 줄어들 가능성까지 감안하기 어렵다**는 걸 보여주는 사례에 가깝다. 지금 이 화면을 보면 오버스펙처럼 보일 수 있다는 것도, 왜 그렇게 됐는지도 둘 다 인지하고 있다.

### 6. 화면 재생성에도 진행 상태 지키기
...
- 1번 섹션의 `MutableSharedFlow` 기반 네비게이션 이벤트는 `replay = 0`이라, 화면이 백그라운드로 내려가는 등 구독자가 없는 순간에 emit되면 그 이벤트는 유실된다. 이런 이벤트를 반드시 보존해야 한다면 `Channel`이나 "소비 여부를 포함한 State"로 바꾸는 게 안전하다.
...
- 아키텍처는 기획 스펙을 기준으로 판단하지만, 기획은 일정에 따라 줄어들기도 한다. 스펙이 확정되기 전에 구조를 정해야 한다면, 이후 스펙이 축소될 가능성까지 감안해 "나중에 되돌리기 쉬운 선택"을 우선하는 것도 방법이다.
