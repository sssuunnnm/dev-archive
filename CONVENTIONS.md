...
> **PR을 올리는 시점이 곧 발행 전환 시점이다.** 작성 중에는 `draft: true`로 두되, PR을 올릴 만큼 완성됐다고 판단하면 그때 `draft: false`로 바꾸고 `date`도 실제 발행일(PR 올리는 날)로 맞춘다. PR이 리뷰 대기 상태인 동안 `draft: true`로 남겨두지 않는다 — merge 이후 별도로 발행 전환 커밋을 만드는 방식은 지양한다.
...
- **단, 같은 대화(채팅) 세션 안에서 여러 글/작업을 연달아 처리하는 경우에는 세션당 브랜치 하나로 몰아서 작업한다.** 글마다 브랜치를 쪼개면 세션 하나가 끝날 때 리뷰할 브랜치가 여러 개로 흩어져서 오히려 확인하기 번거롭다. 새 세션에서 이어서 작업할 때(예: 다음 날 자동화, 다른 대화)는 새 브랜치를 판다.
